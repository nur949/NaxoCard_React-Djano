...
export default function MiniCartDrawer() {
  ...
  return (
    <>
      ...
      />
      <aside className={`fixed right-0 top-0 z-[71] flex h-dvh w-full max-w-sm flex-col bg-card shadow-premium transition-transform duration-300 ${drawerOpen ? "translate-x-0" : "translate-x-full"}`} aria-label="Mini cart">
        ...
        <div className="flex-1 overflow-y-auto p-3.5">
          {cart.items.length === 0 ? (
            ...
            <div className="grid gap-3">
              {cart.items.map((item) => (
                <div key={item.id} className="animate-fade-in rounded-lg border p-2.5">
                  <div className="flex gap-3">
                    <img className="h-16 w-16 rounded-md bg-white object-contain" src={productImage(item.product, "https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?auto=format&fit=crop&w=300&q=80")} alt={item.product.name} />
